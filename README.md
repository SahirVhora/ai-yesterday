...
You can override the model with a repository variable named `OPENROUTER_MODEL`. If a paid model is unavailable, the collector retries `openrouter/free`. The scheduled job enriches only the top few items and stops on rate limits so it does not burn through the free quota. If no key is present, the collector falls back to rules-based summaries.
